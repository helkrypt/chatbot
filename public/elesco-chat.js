(function () {
    ...
    function init() {
        ...
        if (document.getElementById('elesco-chat-bubble')) return;
        build();
    }
    ...
    function build() {
        ...
        style.textContent = [
            ...
            '}',
            '#elesco-chat-bubble:hover{transform:scale(1.08);}',
            '#elesco-chat-bubble.elesco-hidden{',
            'opacity:0!important;pointer-events:none!important;transform:scale(0.6)!important;',
            '}',
            '#elesco-chat-bubble svg{',
            ...
            'white-space:nowrap;',
            '}',
            '#elesco-chat-tooltip::after{',
            'content:"";position:absolute;bottom:-7px;right:22px;',
            'width:14px;height:14px;background:#fff;',
            'transform:rotate(45deg);box-shadow:3px 3px 6px rgba(0,0,0,0.08);',
            '}',
            '#elesco-chat-tooltip.elesco-hidden{opacity:0!important;pointer-events:none!important;transform:translateY(6px);}',
            /* Iframe wrapper — responsive + safe-area */
            '#elesco-chat-iframe-wrap{',
            'position:fixed!important;',
            ...
            '}',
            '#elesco-chat-iframe-wrap.elesco-hidden{',
            'opacity:0!important;pointer-events:none!important;',
            'transform:scale(0.85) translateY(20px)!important;',
            '}',
            '#elesco-chat-iframe-wrap iframe{',
        ...
        wrap.id = 'elesco-chat-iframe-wrap';
        wrap.classList.add('elesco-hidden');
        ...
        function openChat() {
            isOpen = true;
            wrap.classList.remove('elesco-hidden');
            bubble.classList.add('elesco-hidden');
            hideTooltip();
            try { iframe.contentWindow.postMessage({ type: 'elesco-open' }, '*'); } catch (e) { }
        }

        function closeChat() {
            isOpen = false;
            wrap.classList.add('elesco-hidden');
            bubble.classList.remove('elesco-hidden');
        }

        /* ── TOOLTIP ────────────────────────────────────────────────────────── */
        var tooltipTimer;

        function showTooltip() { tooltip.classList.remove('elesco-hidden'); }
        function hideTooltip() { tooltip.classList.add('elesco-hidden'); clearTimeout(tooltipTimer); }
        ...
        window.addEventListener('message', function (event) {
            ...
            if (data.type === 'elesco-close') closeChat();
            if (data.type === 'elesco-open') openChat();
        });
        ...
        setTimeout(function () {
            var b = document.getElementById('elesco-chat-bubble');
            if (!b) {
                // Something removed our element – rebuild once
                window.__elescoLoaded = false;
                init();
                return;
            }
            // Force visibility in case a WP plugin hid it
            if (!isOpen) {
