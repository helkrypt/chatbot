...
async function buildClientReport(clientId, clientName, startStr, endStr, monthName, periodeTekst, now) {
...
export async function GET(request) {
  try {
    ...
    const { data: existingLog } = await supabase.from('cron_log')
      .select('id, status').eq('job_name', 'monthly-report').eq('run_date', todayStr).single();
    ...
    const { data: logRow } = await supabase.from('cron_log')
      .upsert({ job_name: 'monthly-report', run_date: todayStr, status: 'running' }, { onConflict: 'job_name,run_date' })
      .select('id').single();
    ...
    for (const client of clients || []) {
      ...
      const { html, stats } = await buildClientReport(client.id, client.name, startStr, endStr, monthName, periodeTekst, now);

      await sendEmail({
        to: recipient,
        subject: `${client.name} Chatbot - Månedlig rapport (${monthName})`,
        html,
        clientId: client.id,
      }).catch(err => console.error(`[monthly-report] Feil ved sending til ${client.id}:`, err));
